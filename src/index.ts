...
export class OrbitControls extends THREE.EventDispatcher {
    ...
    constructor(object: THREE.Camera, domElement?: HTMLElement, domWindow?: Window) {
        ...
        this.onMouseWheel = (event: any) => {
            ...
            mouse.x = (event.offsetX / (element as any).offsetWidth) * 2 - 1;
            mouse.y = -(event.offsetY / (element as any).offsetHeight) * 2 + 1;
            ...
            if (event.deltaY !== undefined) { // WebKit / Opera / Explorer 9
                var preDelta =  event.wheelDelta > 20 || event.wheelDelta < -20 ? event.wheelDelta / 6 : event.wheelDelta;
            ...
            if ( pos0.length() < this.maxDistance) {
                this.target.add(vec3dir);
                this.object.position.add(vec3dir);
            }
            /* else {
                if (delta < 0) {
                    this.dollyOut(this.getZoomScale());
